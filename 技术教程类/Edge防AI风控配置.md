...
干净程度：我的IP不够干净，但可以让“设备”更加干净。于是设置浏览器，专门用1个Profile来登录Gemini、Claude（每个Profile就可以模拟一个设备），保持AI平台检测到的设备指纹固定、比较干净，再辅助一些网络设置降低检测风险。这样就算换了IP也不需要担心
...
## 4、备注
（1）新的Profile用普通窗口就行，无痕浏览不会保留Cookie、关了后需要重新登录
